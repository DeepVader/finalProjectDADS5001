...
def load_landmarks():
    with duckdb.connect("db/airbnb.db") as con:
        df = con.query("SELECT * FROM bkk_dist").df()
    return df
...
def load_data():
    ...
    data = data.dropna(
        subset=["price", "review_scores_rating", "latitude", "longitude"]
    )
...
def build_where_clause(
    neighbourhoods, price_ranges, property_groups, room_types, amenities
):
    conditions = []
    ...
    if price_ranges:
        ...
        for low, high in price_ranges:
            price_conds.append(
                f"(CAST(REPLACE(price, '$', '') AS DOUBLE) BETWEEN {low} AND {high})"
            )
...
if "neighbourhood" in data.columns:
    selected_neighbourhood = st.multiselect(
        "Select neighbourhood", data["neighbourhood"].dropna().unique()
    )
...
if "property_type" in data.columns:
    conditions = [
        ...
        data["property_type"].str.lower().str.contains("condominium", na=False)
        | data["property_type"].str.lower().str.contains("condo", na=False),
...
selected_property_groups = st.multiselect(
    "Select Property Type Grouped", property_groups
)
...
if all(
    col in filtered_data.columns for col in ["latitude", "longitude", "neighbourhood"]
):
...
if not filtered_data.empty and "neighbourhood" in filtered_data.columns:
    ...
    summary = (
        filtered_data.groupby("neighbourhood")
        .agg(
...
if lat_col and lng_col:
    ...
    selected_landmark = st.selectbox(
        "Selected Landmark", landmark_names, key="landmark_select"
    )
    ...
    st.dataframe(
        top_nearest[
            ["name", "neighbourhood", "total_price", "review_scores_rating", dist_col]
        ]
    )
    ...
    st.plotly_chart(
        px.scatter(
            ...
            hover_name="name",
            labels={
                dist_col: f"Distance to {selected_landmark} (km)",
                "total_price": "Price (THB)",
            },
    ...
    top_nearest[dist_str_col] = top_nearest[dist_col].apply(
        lambda x: f"{x:.2f} km" if pd.notna(x) else "N/A"
    )
