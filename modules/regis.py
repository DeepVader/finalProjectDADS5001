def show(collection):
    ...
    # ----- Clear Form Trigger -----
    if (
        "clear_form_triggered" in st.session_state
        and st.session_state.clear_form_triggered
    ):
        ...
        for key in list(st.session_state.keys()):
            if key.startswith("amenity_") or key in (
                ...
                st.session_state[key] = (
                    "" if isinstance(st.session_state.get(key), str) else 0
                )
            if key == "select_all":
    ...
    minimum_nights = st.number_input(
        "Minimum Nights", min_value=1, key="minimum_nights"
    )
    ...
    new_amenity_input = st.text_input(
        "Other Amenities (comma separated)", key="other_amenities"
    )
    if new_amenity_input and new_amenity_input.strip():
        custom_amenities = [
            a.strip().title() for a in new_amenity_input.split(",") if a.strip()
        ]
    ...
    if not latest.empty:
        ...
        if "amenities" in latest.columns:
            latest["amenities"] = latest["amenities"].apply(
