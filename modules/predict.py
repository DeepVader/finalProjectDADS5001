def show(df):
    ...
    amenity_cols = [
        ...
        "Washer",
        "Wifi",
    ]  # กำหนดคอลัมน์สิ่งอำนวยความสะดวก

    # --- ส่วนติดต่อผู้ใช้ (UI) สำหรับรับข้อมูล ---
    st.subheader("📌 Input for Prediction")
    selected_neighbourhood = st.selectbox(
        "Select Neighborhood:", sorted(df["neighbourhood"].dropna().unique())
    )
    selected_room_type = st.selectbox(
        "Select Room Type:", sorted(df["room_type"].dropna().unique())
    )
    selected_property_grouped = st.selectbox(
        "Select Property:", sorted(df["property_grouped"].dropna().unique())
    )
    ...
    with amenity_col1:
        for i, col in enumerate(amenity_cols[: len(amenity_cols) // 2]):
            checked = st.checkbox(
                col.replace("_", " ").title(), key=f"amenity1_{i}", value=select_all
            )
            if checked:
    ...
    with amenity_col2:
        for i, col in enumerate(amenity_cols[len(amenity_cols) // 2 :]):
            checked = st.checkbox(
                col.replace("_", " ").title(), key=f"amenity2_{i}", value=select_all
            )
            if checked:
                selected_amenities.append(col)
    ...
    X_dummy = pd.get_dummies(
        X_raw, columns=categorical_cols, drop_first=True
    )  # ทำ One-hot encoding ให้คอลัมน์ประเภท
    ...
    for c in categorical_cols:
        ...
        for v in df[c].dropna().unique():
            ...
            if col_name_dummy in feat_selected:
                ...
                input_dict[col_name_dummy] = [
                    0
                ]  # กำหนดเป็น 0 หากคอลัมน์นี้ไม่ได้ถูกเลือกใน feat_selected
        ...
        with st.spinner("Processing..."):
            ...
            if model_choice == "Random Forest":
                if perform_tuning:
                    ...
                    param_grid = {
                        ...
                        "max_depth": [
                            ...
                            10,
                            None,
                        ],  # ความลึกสูงสุดของต้นไม้ (None คือไม่จำกัด)
                    ...
                    st.write(
                        f"Random Forest Best Hyperparameters: {grid_search.best_params_}"
                    )
                else:
                    model_to_fit = RandomForestRegressor(
                ...
                if perform_tuning:
                    ...
                    model_to_fit = grid_search.best_estimator_
                    st.write(
                        f"Decision Tree Best Hyperparameters: {grid_search.best_params_}"
                    )
                else:
                ...
                if perform_tuning:
                    ...
                    st.write(
                        f"XGBoost Best Hyperparameters: {random_search.best_params_}"
                    )
                else:
                ...
                if perform_tuning:
                    st.warning(
                        "Linear Regression does not significantly benefit from hyperparameter tuning."
                    )

            # ตรวจสอบว่าโมเดลถูกกำหนดแล้ว หากเกิดข้อผิดพลาดจะหยุดการทำงานตรงนี้
            if model_to_fit is None:
                st.error(
                    "Error: Could not create model. Please select a model and retry."
                )
                st.stop()  # ใช้ st.stop() เพื่อหยุดการทำงานของสคริปต์ใน Streamlit

            # ทำ Cross-Validation และทำนายด้วยโมเดลที่ได้
            scores = cross_val_score(
                model_to_fit, X_train, y, cv=3, scoring="neg_mean_absolute_error"
            )
            mae = -np.mean(scores)
            ...
            df_cluster = df[df["neighbourhood"] == selected_neighbourhood][
                used_cols
            ].copy()
            df_cluster["price"] = df[df["neighbourhood"] == selected_neighbourhood][
                "price"
            ]
            ...
            combined_df = pd.concat(
                [df_cluster, temp_input_cluster], ignore_index=True
            ).fillna(0)
            ...
            # หาคู่แข่ง
            competitors = df_cluster[
                df_cluster["cluster"] == input_cluster_label
            ].copy()
            ...
            st.write(
                f"🔍 Filtering competitors only in neighbourhood: `{selected_neighbourhood}`"
            )
